...
Description: AIT 726 Homework 2 - Part1 
...
###########Results###############
  For stem: Accuracy is 0.87
            Confusion matrix  [[2792  208]
                              [ 341  841]]
      
  For non-stem: Accuracy is 0.9
                Confusion matrix [[2794  206]
                                 [ 226  956]]
	 
Detailed run results of the program along with the metrics for training loss, validation loss and validation accuracy 
are attched as SentimentClass_FFNN_out.log file 
  '''
...
def TrainingAndCV(train_data, train_labels, n_splits = 2):           
    ...
    for i, (train_idx, valid_idx) in enumerate(splits):    
        ...
        for lr in lrs:
            lrdict[lr] = train_pred_ModelTuning(model, x_train, y_train, x_val, y_val, lr, epochs=200, batch_size=400)
...
def main():
  ...
  print("\t ------------------------------------- \n \t For stem dataset \n \t -------------------------------------")
  model_stem = TrainingAndCV(traindf_stem_tfidf, train.Sentiment, n_splits = 4)
  print("\t ------------------------------------- \n \t For non stem dataset \n \t -------------------------------------")
  model_nostem = TrainingAndCV(traindf_nostem_tfidf, train.Sentiment, n_splits = 4)
